...
1. **Pipelines** — top layer. The user's full project: hardware bindings, data bindings, recipes used, parameter overrides, interaction surface (TBS / GUI / formal notations / script). Compiles to a `.tam` IR + per-pass per-door kernel binaries.
2. **Recipes** — every named composition. Flat. One file per recipe. Examples: `mean`, `correlation`, `log`, `exp`, `parkinson_volatility`, `kyle_lambda`, `vpin`, `efa`, `garch_fit`, `sip_signal_bundle`. Recipes contain NO inline arithmetic — they compose atoms + primitives (via Op/Expr) + other recipes. Defaults are tambear-best-practice; users override via `using()`.
3. **Atoms** — exactly two: `accumulate(grouping, expr, op)` and `gather(addressing, ...)`. Everything compiles down to these.
4. **Op + Expr** — IEEE-level dials parameterizing the atoms. `Op::Add`, `Op::Max`, `Op::Min`, `Expr::Value`, `Expr::Custom("v*w")`, etc. Live as enums inside `crates/tambear/src/accumulate.rs`.
5. **Primitives** — low-level implementation machinery: hardware ops (`fadd`, `fmul`, `fmadd`, `fsqrt`, `fmin`, `fmax`), compensated arithmetic (`two_sum`, `two_product_fma`, `kahan_sum`), DoubleDouble type, Kulisch accumulator, sum_k. Lives at `crates/tambear/src/primitives/`. The compiler uses these to lower Op/Expr per backend.

Older sections of this CLAUDE.md and other docs across the repo may use words like *primitive*, *method*, *specialist*, *operation*, *layer* with different meanings. The mapping table is in `vocabulary.md`. Where any reading of this document conflicts with `vocabulary.md`, that document wins.
...
### Tier-distinct mathematical structure — recipes are content-addressed, the IR is provenance-addressed

Tambear's recipe tier (Tier 4) and the tiers below it are **holonomic and content-addressed** — same bag of (parameter, value) pairs → same compiled kernel. The cache key asks *"have I seen this thing before?"* Order of binding doesn't matter; lineage doesn't matter; only the assignments matter. Cache keys operationalize this: stable tag bytes per parameter slot, deterministic key from assignments. *A recipe will taste the same regardless of which ingredients you put in when.*

The IR layer that compiles pipelines into kernel binaries is **non-holonomic and provenance-addressed** — same values + different pipeline context → different compiled kernel. The cache key asks *"have I done this thing before?"* The address encodes HOW the result was produced, not WHAT the result contains. Two pipelines that include recipe A but have different sharing contexts produce different compiled kernels under different provenance keys. By design — cache correctness is structural, not contractual. Some ingredients need to be cooked in a separate pot because they don't blend with others. The IR is a stew, not a steak; live performance art, not a heuristic list.

**The two cache disciplines already exist** in tambear (provenance-addressing was operationalized in past-Claude's March 30 garden entry, two months before the lens named it). The lens is the *test* that says which discipline applies where: apply to a parameter slot — pass (content-addressable) → recipe-tier tool applies; fail (provenance-required) → IR-tier tool applies.

**Why irrevocable**: forcing the IR into content-addressed mold (collapsing different lineages into the same key) loses the global-context information that's exactly what the IR needs to do its job. Forcing the recipe tier into provenance-addressed mode (caching by HOW recipes are reached) gives up the composability that makes "all of math" tractable — same recipe in different files would miss cache hits despite identical parameters. Mixing the tools breaks both. See `docs/architecture/holonomic-architecture.md`.
...
| Recipe | Recipes (and atoms) it composes | What the recipe uniquely owns |
|--------|----------------------|-------------------------------|
| `kendall_tau` | `sort`, `inversion_count`, `tie_count` | tau-b formula |
| `pearson_r` | `mean`, `variance`, `covariance` | r = cov/(sx*sy) |
...
| `garch_fit` | `garch_filter`, `log_likelihood`, `optimizer` | variance recursion equation |
