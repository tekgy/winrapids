...
**ETF arbitrage hypothesis for KO/MSFT anomaly**: KO is a large constituent of XLP, VIG, SDY,
and most value/dividend baskets. MSFT is ~7-12% of SPY/QQQ/VTI. ETF arb desks execute
creation/redemption baskets at ~5min cadences (iNAV arbitrage band monitoring). When an ETF
arb basket executes, ALL constituents trade simultaneously — producing correlated inter-arrival
periodicity at the ETF's execution cadence across all basket members.

**Two distinct 5-minute mechanisms**:
- *Options gamma*: NVDA, TSLA, META — gamma desk recalibration, predicts VOL-related outcomes
- *ETF basket arb*: KO, MSFT, JNJ, BRK.B — arb execution cadence, predicts basket-relative returns

AAPL spans both (largest ETF weight AND massive options OI); may be why it reads lower (4.1x)
than either mechanism alone would predict — two clocks at slightly different phases averaging down.
CHWY (1.8x): no meaningful ETF weights, no options market. Pure execution noise baseline.

**Testable via K04**: KO-MSFT 5min excess correlation should be HIGH (shared ETF arb mechanism).
KO-NVDA correlation should be LOW (different mechanisms). If AAPL clusters with KO, ETF
dominates; if with NVDA, options dominates. See garden: 2026-03-28-ko-anomaly-and-etf-arbitrage.md
...
The intraday pulsation profiles are per-ticker and qualitatively distinct:
- **AAPL pattern**: late-morning burst (segment 1: 85.1x), collapse at midday (segment 2: 0.8x)
- **NVDA pattern**: continuously high, monotonically declining through the day (76.7x → 28.5x)

This temporal fingerprint — *when* algorithms are active, not just *how much* on average — is
information that full-day FFT averages away. **K02 KO04 (wavelet scalogram at 5min scale)
is the natural representation**: it captures both the oscillation AND the amplitude envelope
across the session. This is a second, independent justification for K02 KO04 beyond Layer D
(daily trend). Cross-date stability of these temporal profiles is untested but plausible.
...
**What does not survive as a reliable signal**:
...
**K02 KO05 PHASE leaf — production, V columns carry confidence** (2026-03-28):
The farm computes PHASE for every ticker, every day, regardless of stability. V columns carry
the signal about the signal — consumers decide what to trust.

Schema (per target period, separate KO05 file per cadence):
```
DO01: phase_cos        ← cos(phase) at target period
DO02: phase_sin        ← sin(phase) at target period
DO03: phase_excess     ← spectral excess (detrended) — the robust feature
V01:  within_day_r     ← phase stability (0=junk, 1=locked)
V02:  bootstrap_ci_deg ← 95% CI width in degrees
V03:  n_segments_used  ← how many intraday segments contributed
```

V01=0.15 on most 5min days (unstable). V01=0.85 on locked days. The amplitude (DO03) is
always meaningful. The phase (DO01, DO02) is meaningful when V01 is high.

Future investigation: shorter-window phase estimates, conditioned on high-amplitude segments.
Execution regime (1-30s) phase stability not yet tested — may show higher V01.
...
**Note on K02 KO04**: Wavelet is justified for TWO layers, not one:
- Layer D: daily trend (the original motivation — DFT aliases the U-shape into spurious harmonics)
- Layer C: amplitude pulsation envelope (AAPL late-morning burst, NVDA monotonic decline)

The scalogram at 5min scale captures both simultaneously. This is a stronger architectural case
for K02 KO04 than Layer D alone.
