...
## Experiment 18: Column Bundling — 25 Narrow vs 5 Wide vs 1 Mega

**Date**: 2026-03-28
**Motivation**: KO05 files have 25 stat columns (5 data cols × 5 stats) with 4096-byte alignment each = 110 KB minimum per file. Pathmaker measured 1.7ms per file write (Python). I estimated column bundling (25→5) would give 5x speedup. Was I right?

**Setup**: MKTF writer with matched data (7,800 bytes), varying column layout. 30 runs per point.

### Phase 1: Write Speed

| Bins | 25 narrow | 5 wide | 1 mega | Speedup (25→5) |
|---|---|---|---|---|
| 1 (session) | 1.68ms / 108 KB | 0.97ms / 28 KB | 0.77ms / 12 KB | 1.7x |
| 13 (30min) | 1.71ms / 108 KB | 1.06ms / 28 KB | 0.91ms / 12 KB | 1.6x |
| 78 (5min) | 1.66ms / 108 KB | 1.09ms / 28 KB | 1.01ms / 16 KB | 1.5x |
| 390 (1min) | 1.91ms / 108 KB | 1.08ms / 48 KB | 1.09ms / 48 KB | 1.8x |
| 780 (30s) | 1.85ms / 108 KB | 1.54ms / 88 KB | 1.07ms / 88 KB | 1.2x |
| 2,340 (10s) | 1.87ms / 308 KB | 1.52ms / 248 KB | 1.11ms / 240 KB | 1.2x |

**Speedup is 1.2-1.8x, not 5x.** My estimate was wrong — the per-file floor dominates.

### Phase 2: Write Time Decomposition (78 bins)

| Layout | Total | Floor (1-col file) | Column overhead |
|---|---|---|---|
| 25 narrow | 1.54ms | 0.73ms | 0.81ms (0.034ms/col) |
| 5 wide | 1.62ms | 0.88ms | 0.74ms |
| 1 mega | 0.79ms | 1.14ms | -0.35ms* |

\* Negative because 1-mega uses different code paths. The floor measurement varies by ~0.4ms.

**Per-column overhead**: ~0.034ms. Saving 20 columns saves ~0.68ms (matches measured 1.71-1.03 = 0.68ms). But the floor at ~0.7-1.0ms (pack_block0, SHA-256, file I/O) is 40-60% of total write time and cannot be reduced by column bundling.

### Phase 3: Read Speed (78 bins)

| Layout | Read Time | File Size |
|---|---|---|
| 25 narrow | 145us | 108 KB |
| 5 wide | 91us | 28 KB |
| 1 mega | 71us | 16 KB |

Read speed tracks file size, not column count. Bundling helps reads too (1.6x faster).

### Phase 4: Universe Projections

| Layout | Per-file | 8 cad (36,832 files) | 10 cad (46,040 files) |
|---|---|---|---|
| **25 narrow** | **1.71ms** | **63s** | **79s** |
| **5 wide** | **1.03ms** | **38s** | **47s** |
| 1 mega | 1.13ms | 42s | 52s |

5 wide is the sweet spot — faster than 1 mega (surprising, possibly due to smaller individual tobytes calls) while preserving per-data-column read selectivity.

### Why My Estimates Were Wrong

| Estimate | Predicted | Measured | Error |
|---|---|---|---|
| Original (pipeline synthesis) | ~0.2ms/file → 9s | 1.7ms → 63s | **8.5x** |
| Bundling speedup (first) | 5x → ~15s | 1.7x → 38s | **2.5x** |
| Bundling speedup (revised) | 2.5-3x → ~22s | 1.7x → 38s | **1.7x** |

Root cause: I modeled column overhead as the dominant cost. It's not — the per-file floor (pack_block0, SHA-256, file open/close) is 40-60% of write time and doesn't scale with column count. Column bundling only addresses the 40-60% that IS column-dependent.

### Design Recommendation

**5 wide columns is the right layout for KO05 files.** It gives 1.7x write speedup, 1.6x read speedup, and preserves per-data-column selectivity. But KO05 writes at 38s (8 cad) are STILL the pipeline bottleneck — larger than reads (13.4s) + GPU (9s) combined.

**Rust writer is the only path to read-bound pipeline.** The Python per-file floor at ~1ms cannot be optimized further in Python. Rust at ~0.1ms/file would make KO05 writes 3.7s → pipeline becomes read-bound at 13.4s.

Benchmark script: `experiments/file_format/bench_column_bundling.py` (runs via fintek venv)
...
**Date**: 2026-03-28
**Motivation**: Team-lead corrected the column bundling approach: "node === node means one column per output, always." The fix is `alignment=64` for KO05 files (GPU cache line) vs `alignment=4096` (default, NVMe page). The MKTF writer already accepts the alignment parameter. How much does this help?

**Setup**: 25 KO05 stat columns (5 data cols × 5 stats), 30 runs per point, safe=False. Same writer, same reader, same columns — only alignment changes.

### Phase 1: Write Speed

| Bins | align=4096 | align=64 | Speedup |
|---|---|---|---|
| 1 (session) | 1.63ms / 108.0 KB | 1.33ms / 8.7 KB | 1.22x |
| 13 (30min) | 1.65ms / 108.0 KB | 1.35ms / 8.7 KB | 1.23x |
| 78 (5min) | 1.52ms / 108.0 KB | 1.38ms / 14.9 KB | 1.10x |
| 390 (1min) | 1.66ms / 108.0 KB | 1.47ms / 46.2 KB | 1.12x |
| 780 (30s) | 1.68ms / 108.0 KB | 1.66ms / 83.7 KB | 1.01x |
| 2,340 (10s) | 2.02ms / 308.0 KB | 1.93ms / 236.8 KB | 1.05x |

**Write speedup is modest: 1.01-1.23x.** The per-file floor (0.87ms) is 56-63% of total write time and is alignment-independent. At 780+ bins where data dominates padding, the two alignments converge.

### Phase 2: File Size Analysis

| Bins | align=4096 | align=64 | Size reduction | Utilization (4096 → 64) |
|---|---|---|---|---|
| 1 (session) | 108.0 KB | 8.7 KB | **12.4x** | 0.1% → 1.1% |
| 78 (5min) | 108.0 KB | 14.9 KB | **7.2x** | 7.1% → 51.0% |
| 780 (30s) | 108.0 KB | 83.7 KB | **1.3x** | 70.5% → 91.0% |

**File size is the big win.** At 78 bins (representative 5min cadence): 108 KB → 14.9 KB. The 110 KB alignment penalty at 4096 bytes was absurd for KO05's small columns — each column got padded to 4096 bytes regardless of data size.

### Phase 3: Write Time Decomposition (78 bins)

| Alignment | Total | Floor | Column overhead |
|---|---|---|---|
| 4096 | 1.69ms | 0.87ms | 0.83ms |
| 64 | 1.48ms | 0.87ms | 0.62ms |

Floor is identical (0.87ms) — pack_block0, SHA-256, file I/O are alignment-independent. Column overhead drops 25% (0.83→0.62ms), likely from less data to hash and write. The floor is 59% of align=64 total.

### Phase 4: Read Speed (78 bins)

| Alignment | Read Time | File Size |
|---|---|---|
| 4096 | 133us ±10 | 108.0 KB |
| 64 | 105us ±10 | 14.9 KB |

**21% faster reads.** Less data to read from disk. At 7.2x smaller files, the NVMe bandwidth savings are real.

### Phase 5: Universe Projections

| Configuration | Per-file | 8 cad (36,832 files) | 10 cad (46,040 files) |
|---|---|---|---|
| **align=4096** | **1.65ms** | **60.6s** | **75.8s** |
| **align=64** | **1.54ms** | **56.9s** | **71.1s** |

**Only 6% write time reduction at universe scale.** The Python per-file floor (0.87ms = 56% of write time) renders the alignment change nearly irrelevant for write throughput.

### Phase 6: Correctness

Both alignments roundtrip all 25 columns perfectly. `header.alignment` reads back correctly at 4096 and 64 respectively.

### Conclusions

alignment=64 is the **correct architectural fix** for KO05 files:
1. **File size: 7.2x smaller** at 78 bins (108 KB → 14.9 KB) — huge storage win
2. **Disk utilization: 51% vs 7.1%** — files contain mostly data, not padding
3. **Read speed: 21% faster** (133us → 105us) — less wasted I/O
4. **Correctness: verified** — same format, same writer, parameter only

But it does **NOT solve the write speed problem**:
- Write speedup: 1.10x at 78 bins (1.52ms → 1.38ms)
- Universe: 60.6s → 56.9s (6% reduction)
- **Python per-file floor (0.87ms) is 56-63% of write time** — alignment cannot touch it
- **Rust writer (~0.1ms) remains the only path to read-bound pipeline**

### Comparison: Bundling (Exp 18) vs Alignment (Exp 19)

| Fix | Write speedup | File size | Read speedup | node===node? |
|---|---|---|---|---|
| Column bundling (25→5) | 1.7x | 28 KB | 1.6x | **NO** (violates) |
| Alignment=64 | 1.10x | 14.9 KB | 1.3x | **YES** (preserves) |

Alignment=64 produces **smaller files** than bundling (14.9 KB vs 28 KB) while preserving the one-column-per-output architecture. Write speed is slower than bundling, but both approaches are dominated by the per-file floor.

Benchmark script: `experiments/file_format/bench_ko05_alignment.py` (runs via fintek venv)

---

## Experiment 20: Bulk Write/Read Scaling — Do Micro-Benchmarks Compose?

**Date**: 2026-03-28
**Motivation**: All prior experiments measured single-file performance (30 runs of the same file). The pipeline model multiplies per-file time × file count and assumes linearity. This experiment tests that assumption with 2000 distinct files written and read in sequence.

**Setup**: 2000 unique KO05 files (25 columns, 78 bins, alignment=64, safe=False), timed in batches of 100. Each file has different data (different RNG seeds). Pre-generated column data to exclude generation time.

### Phase 1: Bulk Write — Linearity Validated

| Batch | Per-File | Drift from Baseline |
|---|---|---|
| 0-99 | 1.450ms | 1.00x |
| 200-299 | 1.429ms | 0.99x |
| 500-599 | 1.566ms | 1.08x |
| 900-999 | 1.581ms | 1.09x |
| 1200-1299 | 1.324ms | 0.91x |
| 1900-1999 | 1.382ms | 0.95x |

**Total: 2.88s for 2000 files = 1.438ms/file.** Drift range: 0.91-1.09x (noise, no trend). Exp 19 single-file: 1.38ms. Ratio: **1.04x — writes compose linearly.** The pipeline write model is validated.

### Phase 2: Directory Structure — No Effect

| Structure | Per-File |
|---|---|
| Flat (1 directory) | 1.363ms |
| Sharded (10 subdirs) | 1.413ms |

NTFS handles 2000 files in one directory with no degradation.

### Phase 3: Bulk Read — 46x Worse Than Single-File Benchmark

| Batch | Per-File | Drift |
|---|---|---|
| 0-99 | 4.774ms | 1.00x |
| 500-599 | 4.556ms | 0.95x |
| 1000-1099 | 4.756ms | 1.00x |
| 1900-1999 | 4.803ms | 1.01x |

**Total: 9.75s for 2000 files = 4,876us/file.** Exp 19 single-file warm: 105us. Ratio: **46.4x — reads do NOT compose.**

Drift is stable (0.95-1.09x) — the degradation is per-file overhead, not cumulative. Each unique file costs ~4.9ms regardless of position.

### Root Cause Analysis

The single-file benchmark (Exp 19) warmed with 5 reads of the SAME file, then timed 30 more reads of the same file. Everything was cached — OS page cache, NTFS MFT entries, Python buffer state.

Bulk reads of unique files hit per-file overhead that doesn't exist for repeated reads:

1. **Double file open**: `read_columns()` calls `read_header(path)` which opens/reads/closes, then opens the file AGAIN to read column data. That's **2 × CreateFile + 2 × CloseHandle per file = 4,000 syscalls for 2000 files.**

2. **Per-column seeks**: The reader does 25 individual `f.seek()` + `f.read()` operations for column data, even though the columns are contiguous in a 14.9 KB file. A single `f.read()` + buffer slicing would eliminate 24 seeks.

3. **NTFS metadata overhead**: Each CreateFile involves MFT lookup, security descriptor check, sharing mode validation. For unique files in a 2000-entry directory, MFT index traversal is non-trivial.

**Effective bandwidth**: 2000 × 14.9 KB = 29.8 MB in 9.75s = **3.1 MB/s**. The NVMe does 4,860 MB/s. We're at **0.06% utilization** — this is pure overhead.

### Fix Recommendations (for pathmaker)

Two changes to `reader.py` would largely eliminate the 46x gap:

1. **Single file open**: Read the entire file into a buffer with one `f.read()`, then parse header and extract columns from the buffer. Eliminates the double-open.

2. **Buffer slicing instead of per-column seeks**: Once the file is in a buffer, use `np.frombuffer(buf, offset=..., count=...)` instead of seek+read per column. Eliminates 25 seeks.

For a 14.9 KB file, one `f.read()` of the entire file replaces 2 opens + 26 reads + 25 seeks. Expected improvement: 10-30x for small files, converging to 1x for large files where I/O dominates.

### Phase 4: Corrected Universe Projections

| Operation | Micro-bench | Bulk (measured) | Universe 8 cad | Universe 10 cad |
|---|---|---|---|---|
| Write | 1.38ms/file | 1.438ms/file | **53.0s** | **66.2s** |
| Read | 105us/file | 4,876us/file | **179.6s** | **224.5s** |

**The pipeline model was correct for writes but wrong by 46x for reads.** K04 screening (reading 4,604 KO05 files per cadence) would take **22.5s** per cadence, not 0.48s.

### Impact on Pipeline Model

| Estimate | Before Exp 20 | After Exp 20 | Change |
|---|---|---|---|
| K04 screening per cadence | 0.48s | **22.5s** | **46x worse** |
| KO05 reads "nearly free" | Yes | **No** | Architecture cost is real |
| Reads vs progressive (0.4s) | 1.2x slower | **56x slower** | Progressive was much better for screening |
| Read bottleneck rank | #4 (minor) | **#1 for K04** | Reader optimization is critical |

**Key insight**: The reader optimization (single open + buffer slicing) should be part of the Rust writer work, not deferred. For small files, Python reader overhead dominates even more than Python writer overhead.

Benchmark script: `experiments/file_format/bench_bulk_write_scaling.py` (runs via fintek venv)

---

## Experiment 21: Bulk Read Fix — Single-Read + Buffer-Slicing

**Date**: 2026-03-28
**Motivation**: Experiment 20 found bulk reads 46x slower than single-file warm. Root cause: double file open + per-column seeks. This experiment tests a simple fix: one `f.read()` + buffer parsing.

**The fast reader** (standalone, not modifying production code):
```python
def fast_read_columns(path):
    with open(path, "rb") as f:
        buf = f.read()  # ONE syscall
    ...
    for entry in header.columns:
        columns[entry.name] = np.frombuffer(
            buf, dtype=..., offset=entry.data_offset, count=entry.n_elements
        ).copy()
...
| Scenario | Production | Fast Reader | Speedup |
|---|---|---|---|
| Single-file warm (30 runs) | 116us ±22 | 94us ±6 | 1.2x |
| **Bulk 2000 unique files** | **4,657us/file** | **103us/file** | **45x** |
| Re-read (all cached) | 118us | 99us | 1.2x |

**The fast reader eliminates the cold-file penalty.** 103us cold ≈ 94us warm (1.1x). The production reader has a 40x cold/warm gap.

### Why It Works

The production reader's per-file cost at scale comes from:
1. **Two CreateFile syscalls** (read_header opens/closes, read_columns opens again)
2. **25 ReadFile syscalls** (seek + read per column)
3. **NTFS MFT lookups** for unique files (not cached on first access)

The fast reader does:
1. **One CreateFile** + one ReadFile (entire 14.9 KB file)
2. **Zero seeks** — buffer slicing is memory operations only
3. **Same NTFS lookup** but only once, not twice

The NTFS MFT lookup per unique file is ~50-100us (unavoidable). The production reader pays this twice; the fast reader pays it once. The 25 per-column seeks add ~100us each of Python function call + ReadFile overhead. The fast reader replaces these with np.frombuffer offset slicing at ~1us each.

### Corrected Universe Projections

| K04 Screening | Production Reader | Fast Reader | Exp 19 (warm, wrong) |
|---|---|---|---|
| Per-file | 4,657us | **103us** | ~~105us~~ |
| Per cadence (4,604 files) | 21.4s | **0.47s** | ~~0.48s~~ |
| 8 cadences | 171s | **3.8s** | ~~3.9s~~ |

With the fast reader, the original Exp 19 estimate was accidentally correct — warm single-file performance (105us) matches fast-reader bulk performance (103us). The Exp 19 estimate was wrong for the production reader but right for an optimized one.

### Implications

1. **The fast reader fix is a ~15 line change to `reader.py`** — replace `read_header()` + re-open with single `f.read()` + buffer parsing. No format change, no Rust required.
2. **K04 screening drops from 21.4s to 0.47s per cadence** — making KO05 separate files nearly as fast as embedded progressive (0.4s).
3. **The fix benefits ALL small-file reads**, not just KO05. Any MKTF file under ~100 KB would benefit from single-read approach.
4. **Large files (12+ MB KO00) unaffected** — for those, the per-column seek pattern is fine because I/O dominates.

Benchmark script: `experiments/file_format/bench_bulk_read_fix.py` (runs via fintek venv)
...
| 18 | **Column bundling (25→5→1)** | **1.7x write speedup (not 5x); per-file floor ~0.7-1.0ms dominates; Rust still needed for <0.3ms** |
| 19 | **KO05 alignment (4096→64)** | **7.2x smaller files, 21% faster reads, but only 6-10% faster writes; per-file floor (0.87ms) = 56% of write time** |
| 20 | **Bulk write/read scaling** | **Writes compose linearly (1.04x). Reads do NOT: 4.9ms/file bulk vs 105us warm = 46x gap. Double file open + per-column seeks.** |
| 21 | **Bulk read fix** | **Single f.read() + buffer slicing = 45x faster bulk reads. 103us cold = 94us warm. ~15 line fix to reader.py.** |

**Note**: Experiment 18 (column bundling) was **superseded by Experiment 19** (alignment=64). Team-lead corrected that bundling violates node===node. Alignment=64 produces smaller files (14.9 KB vs 28 KB) while preserving one-column-per-output architecture.

**Note**: Experiments 15-17 benchmarked the embedded progressive section, which has been **superseded by separate KO05 files** (architecture shift: sufficient stats are their own MKTF files, not embedded sections). The experimental data remains valid — small-file I/O economics (Exp 14), Python unpacking overhead (Exp 17), and composability math all transfer to KO05 files.

**CORRECTION (Exp 20)**: K04 screening read estimate was based on warm single-file reads (105us). Bulk reads of unique files are **4,876us/file (46x slower)**. Root cause: `read_columns()` opens each file twice and does 25 individual seek+read operations per file. For N unique files this means 2N file opens + 25N seeks on Windows/NTFS. See Experiment 20 for details and fix recommendations.
...
- Batch mode (KO00 only, no-fsync): ~35s (dominated by reads)
- ~~Batch + progressive: ~270s~~ *(superseded by KO05 architecture)*

**K01→K02 with KO05 files (measured data, updated with Rust benchmarks):**
- KO00 writes: 36,832 files × ~0.3ms = ~11s (batch)
- KO05 writes (Python, align=64): 36,832 × **1.44ms** = **53s** (Exp 20 bulk)
- KO05 writes (Rust, new+rename): 36,832 × 1.86ms = 68.7s (pathmaker bulk)
- KO05 writes (Rust, direct): 36,832 × 1.42ms = 52.3s (pathmaker bulk)
- **KO05 writes (Rust, pre-created overwrite): 36,832 × 0.82ms = 30.1s** (pathmaker bulk)
- ~~KO05 writes (Rust, 0.1ms target): 3.7s~~ **UNREACHABLE — NTFS floor**
- NTFS per-file overhead: ~820-1860us depending on operation (overwrite < create < create+rename)
- Rust compute: **5.5us** (160x faster than Python's ~500us) — NTFS dominates at every operation type
- **Pre-creation strategy**: daemon pre-creates empty files during init → writes become overwrites → 2.3x faster
- KO05 file size: 108 KB (align=4096) → **14.9 KB** (align=64) — **7.2x smaller**

**K04 screening via KO05 (CORRECTED by Exp 20/21):**
- Production reader bulk: 4,604 × 4,657us = **21.4s/cadence** (double open + per-column seeks)
- **Fast reader bulk: 4,604 × 103us = 0.47s/cadence** (single f.read + buffer slicing, Exp 21)
- vs full KO00 reads: 0.47s vs 18.7s → **40x faster** (with fast reader)
- vs embedded progressive: 0.47s vs 0.4s → **1.2x slower** — architecture shift is nearly free

**Bottleneck cascade (final, with Rust + pre-creation):**
1. **KO05 writes** remain bottleneck: 53s (Python) / **30.1s (Rust pre-created)** — NTFS is the floor
2. KO00-only batch: **reads** (13.4s of 35s = 38%)
3. With Rust pre-created: writes (30.1s) still > reads (13.4s) — still write-bound
4. **Pipelined wall time**: max(30.1s writes, 13.4s reads) = **~30s** — approaching KO00-only batch
5. **NTFS per-file overhead** (820-1860us/file) is the irreducible constraint
