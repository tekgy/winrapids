...
KO05 files use alignment=64 (CPU cache line) instead of the default
4096 (NVMe sector). 25 columns × 64 bytes = 1,600 bytes overhead
vs 25 × 4096 = 102,400 bytes. One column = one output. node === node.

Usage:
    from winrapids.progressive import extract_ko05_columns, KO05_ALIGNMENT

    columns = extract_ko05_columns(engine, cadence_id=2)
    write_mktf(path, columns, alignment=KO05_ALIGNMENT, ko=5, domain=5, ...)
"""
...
KO05_ALIGNMENT = 64  # CPU cache line — KO05 files are CPU-consumed
...
def extract_ko05_columns(
    ...
    """Extract sufficient statistics as MKTF columns for one cadence.

    One fused kernel launch per source column. Results transferred D2H
    as float32. One output column per (source_column × stat).
    ...
    Returns:
        ...
        Write with alignment=KO05_ALIGNMENT (64) to minimize padding.
    """
