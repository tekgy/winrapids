...
| **P2** | **Faithful-lowering** | Every backend compiles `.tam` → target ISA in a way that pins the IEEE-754 semantics of every emitted op from the target spec alone. The lowering must NOT introduce new ops, reorder existing ops, contract to fused instructions, apply optimization passes that alter bit-level semantics, OR delegate to vendor ops whose behavior is implementation-defined, conditional on optional extensions, or otherwise ambiguous in the target spec. Where a convenient vendor op has ambiguous semantics for the use case (e.g., NaN handling, rounding mode, subnormal treatment), the backend MUST compose from unambiguous primitives and emit that composition unconditionally — no fallback to the vendor op for performance, no conditional selection based on extension availability. The lowering is a homomorphism to a semantically-pinned subset of the target ISA, not a rewrite. | Tambear (backend implementers: CPU, PTX, SPIR-V, ...) |
...
| **Cross-reference** | Enforced at lowering time (`.contract false` in PTX, `NoContraction` in SPIR-V). The IR-level protection is the ABILITY to name the non-contracted form; the lowering-level protection is the DISCIPLINE to emit it. See P2: faithful lowering forbids delegating contraction semantics to vendor defaults — the same principle that requires composing NaN-safe min/max rather than using OpFMin. VB-003 in vendor-bugs.md documents the PTX-specific manifestation (`.contract` default is contract-everywhere). |
...
| **Cross-reference** | Promoted to invariant 2026-04-12 after adversarial bugs P17/P18/P20 plus independent convergence by naturalist + scout. The Aristotle I7 Phase 2 gap (exception/special-value semantics as a fifth computation role) is what connects this to I7's framing. ESC-002 (Vulkan OpFMin/OpFMax NaN undefined) is the canonical P2 application of I11: the SPIR-V backend cannot delegate NaN propagation to OpFMin because OpFMin's NaN behavior is implementation-defined; P2 requires composing from unambiguous primitives instead. See P2: faithful lowering forbids delegating NaN semantics to vendor ops with ambiguous specs. |
