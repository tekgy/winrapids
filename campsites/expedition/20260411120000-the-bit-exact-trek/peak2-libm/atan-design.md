...
Constants from `peak2-libm/libm-constants.toml` (generated by `gen-constants.py` at 100 dps; verified at commit time):

```
; (Adversarial B1 resolution, 2026-04-12: replaced ... placeholders with actual hex)
; Source: libm-constants.toml [piover2_hi], [piover2_mid], [piover2_lo], [pi_over_4_hi], [pi_over_4_lo]

; Two-term π/2 split (for atan2 quadrant additions and atan reassembly to ±π/2):
piover2_hi   = 0x3ff921fb40000000   ; 1.5707962512969970703125  (30 trailing zero mantissa bits)
piover2_mid  = 0x3e74442d00000000   ; 7.549789415861596e-08     (30 trailing zero mantissa bits)
piover2_lo   = 0x3cf8469898cc5170   ; 5.390302858158119e-15     (low correction)
; Note: piover2_hi + piover2_mid + piover2_lo = π/2 with |error| = 8.47843e-32 (< 2^-103)

; Two-term π/4 split (for atan reassembly in the |x| ≤ tan(π/8) regime):
pi_over_4_hi = 0x3fe921fb54442000   ; 0.7853981633970761322416 (12 trailing zero mantissa bits)
pi_over_4_lo = 0x3d5a308d313198a3   ; 3.7217737402433116e-13   (low correction)
; Note: pi_over_4_hi + pi_over_4_lo = π/4 with |error| = 6.2685e-30 (< 2^-97)

; These are NOT equal to piover2_hi/2 and piover2_lo/2 — the trailing-zero structure
; is different: pi_over_4_hi has 12 trailing zeros (optimized for the atan reduction
; interval) while piover2_hi has 30 (optimized for sin/cos range reduction). Use the
; appropriate split for each function.
```
