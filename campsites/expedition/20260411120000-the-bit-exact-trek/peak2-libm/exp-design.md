...
1. **Write `peak2-libm/remez.py`** — Remez minimax fitter using mpmath at 100-digit precision. Input: target function `f(r)`, interval `[a, b]`, degree `d`. Output: polynomial coefficients `[a_0, a_1, ..., a_d]` that minimize `max_{r ∈ [a,b]} |f(r) - P(r)|`, with a certified error bound.
2. **Apply Remez to `(exp(r) - 1 - r) / r²`** on `[-ln(2)/2, ln(2)/2]`, degree 8. This gives `P(r)` such that `exp(r) ≈ 1 + r + r² * P(r)`.
