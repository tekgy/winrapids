...
fn parse_reduce_block_add(ln: usize, toks: &[String]) -> PResult<Op> {
    ...
    if toks.len() < 4 {
        return Err(ParseError::new(ln, "reduce_block_add.f64 requires: %out, %slot, %val"));
    }
    let operands = &toks[1..];
    let (out_buf, slot_idx, val) = parse_three_regs_comma(ln, operands)?;
    Ok(Op::ReduceBlockAdd { out_buf, slot_idx, val })
}
