...
**Path-independence requires path-independent conflict resolution.** When multiple binding sites assign the same parameter slot, the binding bag is reduced via the provenance lattice (`Default < TamOverride < UserOverride` per DEC-020) before serialization. Within a provenance tier, same-value rebinding is idempotent; different-value rebinding is a compile-time error. The cache key depends on `{(slot, final_value)}` — a content-addressable bag — regardless of how many times each slot was bound or in which order. The provenance tier is itself a property of the binding's source (recipe-default vs TAM-analysis vs user-typed), not of its position in the call stack, so composition through nesting preserves holonomicity. See math-researcher's Q1+Q3 walk-through (2026-05-09): `R:\winrapids\campsites\tambear-sweep31-finish\math-researcher\holonomic-q1-q3-walkthrough-2026-05-09.md`.
...
1. **Path-independence formalization** — RESOLVED 2026-05-09. Sufficient *iff* `using()` conflict resolution is itself path-independent (Q3). With the outermost-by-provenance rule per DEC-020 + intra-tier idempotency + compile-time error on intra-tier conflict, the formalization is complete. Sharper formal structure (Q2 partial): the recipe tier's parameter assignments form a **finitely-presented commutative monoid under bag-merge**, and `cache_key` is a **monoid homomorphism**. "Manifold" stays as intuition; "monoid" is the load-bearing structure. See walk-through doc at `R:\winrapids\campsites\tambear-sweep31-finish\math-researcher\holonomic-q1-q3-walkthrough-2026-05-09.md`.
...
3. **`using()` conflict resolution** — RESOLVED 2026-05-09. The principled rule: `using()` bindings form an **idempotent commutative monoid** under value-equality; conflict resolution is outermost-caller-wins per provenance tier (`Default < TamOverride < UserOverride`); intra-tier same-value rebinding is idempotent (no-op); intra-tier different-value rebinding is a **compile-time error** (not silent last-write-wins). Composition through nesting: A's internal `Default` binding loses to any outer override; two `UserOverride` bindings for the same slot in the same composition surface as compile-time error. **Substrate gap (2026-05-09)**: the Provenance enum exists in `using_annotation.rs` and DEC-020 decouples provenance from the cache key correctly — but the conflict-detection logic (Rule R3: error on intra-tier different-value rebinding) does not yet appear to be implemented. The path-independence guarantee is structural-but-not-enforced at HEAD. F13.C graduation condition applies: this antibody must exist at every binding site. Filed as follow-up task. See walk-through doc.
