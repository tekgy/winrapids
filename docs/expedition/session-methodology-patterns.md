...
**Cross-role operational corroboration** (2026-05-19, post-crystallization extension):

| Drift axis (extended) | Instance | X (canonical) | Y (drifted impl) | Author |
|---|---|---|---|---|
| **Doc-comment vs implemented-code** | `clausen.rs` doc-comment formula for unimplemented regime cited wrong leading-order form; correct form is `θ - θ·ln|θ|` from integrating log-sine expansion | The actually-implemented regime status (sibling: code-body) | Doc-comment drifted from impl-truth | Pathmaker (commit `7c978c4`) |
| **Citation framing** (math-researcher's parallel observation) | naturalist commit `7773420` attributed to pathmaker by timestamp adjacency rather than commit-prefix convention | The team's commit-prefix convention (`pathmaker:` / `math-researcher:` / `observer:` prefixes role-author; generic prefixes are sibling agents or Tekgy) | Inferred author from timestamp | Naturalist (self-application, h+1 post-crystallization; logged at `~/.claude/garden/2026-05/2026-05-19-the-pattern-caught-me-within-the-hour.md`) |

The cross-role extension brings Sub-pattern 2.2's operational instance count to **five** (pathmaker's three + clausen.rs + naturalist's self-application). Math-researcher catalogued the cross-role finding in `R:\tambear\campsites\session-20260519\20260518203217-20260519-coordination\math-researcher\insights\20260519-substrate-over-memory-recursive-cross-role-operational-corroboration.md` along with their own 2F1 theorem-memory case (substrate-source = mpmath external oracle; that case is Sub-pattern 5.5 firing rather than Sub-pattern 2.2, since the substrate-source is *external* not *sibling*).

The cross-role corroboration confirms Sub-pattern 2.2 operates across at least three roles (pathmaker, math-researcher, naturalist) and at least five structurally-distinct drift axes (value-flow, vocabulary, decomposition-shape, doc-comment-vs-code, attribution-framing). The pattern is verified team-general and multi-axis at the *sibling-artifact tier*.

**Held forward**: math-researcher (in their bundle `20260519-morning-methodology-candidate-bundle.md`) suggested potentially a generalization beyond Sub-pattern 2.2's sibling-artifact-tier — a *substrate-source axis as discipline-discriminating dimension* meta-pattern catching ALL substrate-source-types (sibling-artifact + external-oracle + canonical-literature + bundle-self-authored-meta-discipline). Naturalist's read: this generalization is *already implicit* in Pattern 2 family structure (canonical Pattern 2 = agent-context substrate-source; Sub-pattern 2.1 = audit-doc substrate-source; Sub-pattern 2.2 = sibling-artifact substrate-source; Sub-pattern 2.3 = session-boundary multi-channel substrate-source; Sub-pattern 5.5 = external-oracle substrate-source firing at antibody-tier rather than X-over-Y-tier). The substrate-source axis IS the family's structural dimension. Crystallizing it as Sub-pattern 2.4 would over-formalize what's already operating; the implicit structure is sufficient. Held as queue substrate-observation, not crystallized.
...
- **Dilogarithm `Li_2(z>1)` sign-bug** (Instance 1, positive-failed-to-deploy): author wrote both impl (line 89) and tests (lines 170-181, 244-247) with the wrong constant `-π²/6` where the correct value is `+π²/3`. The structure-independent canonical check NOT deployed: Spence inversion identity. Three tests, all wrong together; baseline 3017/0/0 passed before the bug was discovered. This is also Sub-pattern 5.5 (orthogonal value-check) trap material; 5.12 names the escape.
- **Airy Wronskian-catches-author** (Instance 2, positive-deployed-and-fired): math-researcher wrote both the anchor formula (§1.2 of Airy P2 notebook 03, with wrong Bi'(-t) sign-pairing) AND would have written the test, but DEPLOYED the canonical Wronskian antibody (`Ai·Bi' − Ai'·Bi = 1/π`) per Olver 1974 prescription. The Wronskian fired at x=-50 with rel=1.165 (essentially unity error) against the author's own anchor. Pathmaker faithfully implemented the wrong-by-author derivation; the structural antibody fired because it was independent of the derivation.
- **Airy hex `u_5` bit-pattern transcription** (Instance 3, negative-failed-to-deploy): math-researcher's anchor for Airy asymptotic coefficients used a manually-transcribed hex table where `u_5` had a transcription error. Bug shipped to pathmaker impl; caught later via independent regeneration. The structure-independent canonical check NOT deployed: inline generation script that the test could regenerate-and-verify against rather than transcribe. This is the **structurally important negative case** — bug shipped because the canonical check (generation-not-transcription) was unnamed-as-required at anchor-write time. (Math-researcher 2026-05-18 evening; documented in F50C.)
- **hyp_1f1 sign-of-Γ via lgamma** (Instance 4, positive-failed-to-deploy): `asymptotic_apply_prefactor` used `lgamma` (= `ln|Γ|`) then `exp()`, dropping the sign when Γ(c-a) is negative. Canonical literature has the sign-of-Γ rule documented for 60+ years (NIST DLMF §5.2.6; Olver §3.2; A&S §6.1.17 — `sign(Γ(x)) = sign(sin(π·x))` for non-integer x via reflection identity). The discipline (when reconstructing Γ from log, always reconstruct sign via reflection identity) was NOT deployed in `asymptotic_apply_prefactor`. Bug shipped; M(0.7, 0.2, -30) returned +0.1075 instead of -0.1243. Caught by scout's hyp_1f1 sign-regime sweep antibody. Different recipe family (hypergeometric) than Instances 1-3 (dilogarithm + Airy), strengthening the cross-family generality of 5.12's discipline. (Math-researcher 2026-05-18 late-evening.)
- **Airy notebook 03 §2.0 inline generation script** (Instance 5, prospective-deployment): math-researcher self-applied 5.12 to their own substrate after the crystallization landed — added an inline generation script ahead of the u_k/v_k hex tables in notebook 03 §2.0, with independent verification. Future readers regenerate the tables rather than transcribe; the tautological-agreement trap that produced the Instance-3 hex bug is closed at the source. **This is the first instance of 5.12 deployed PROSPECTIVELY by an author after the discipline was named** — the methodology pattern transitioning from post-hoc recognition to prospective deployment.

The instances are link-irreducible per Pattern 22.C across multiple axes:
- *Recipe family axis*: dilogarithm (z>1 inversion) vs Airy P2 oscillatory asymptotic vs Airy coefficient tables vs hyp_1f1 hypergeometric — four distinct mathematical families
- *Canonical-structure axis*: Spence inversion (algebraic) vs Wronskian (differential) vs generation-script (computational reproducibility) vs reflection-identity-for-sign-of-Γ (transcendental) — four different canonical-structure types
- *Failure-mode axis*: failed-to-deploy (Instances 1, 3, 4) vs deployed-and-fired (Instance 2) vs prospectively-deployed (Instance 5) — three structural states of the discipline
- *Discovery axis*: external oracle (Instance 1) / deployed antibody (Instance 2) / independent regeneration (Instance 3) / sweep antibody (Instance 4) / N/A — prospective (Instance 5)

The 5-instance trail demonstrates **structural completeness** rather than redundant corroboration — the discipline has been shown to operate at every state of its lifecycle (failed-to-deploy negatives, deployed-and-fired positives, and prospective-deployment closures).
