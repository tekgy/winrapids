...
*Updated at session close with two post-wander findings (scout + navigator, 2026-05-10)*

---

## Post-wander addenda

**EML closure argument** (scout, from `~/.claude/garden/2026-05-10-eml-convergence-sweep35-is-the-luca.md`):

Odrzywolek's EML paper (`reading-notebooks/odrzywolek-eml-luca-elementary-functions.md`) defines `eml(x, y) = exp(x) − ln(y)` as the single binary operator from which ALL elementary functions are reachable by composition from the constant 1. exp(x) = eml(x, 1); ln(x) = eml(1, eml(eml(1,x), 1)); the entire scientific-calculator repertoire follows.

The sweep ordering in this doc is the EML tree made concrete, sweep by sweep. The dependency structure (erfc consumes ExpKernelState; gamma calls .ln() and .exp(); Bessel Y0 has a log(x/2) piece; Li_2 uses log1p) is not accidental — it's the EML composition graph. Sweep 35 implemented the LUCA. Every downstream sweep is a corollary.

The two substrates compose: EML says what elementary functions EXIST (the catalog); tambear's accumulate+gather says HOW they COMPUTE (the execution pattern). They don't compete — they're orthogonal layers.

**erfc is a three-shapes composite** (scout, cross-connect with naturalist's `2026-05-10-the-three-shapes-of-complementary-argument.md`):

Using naturalist's three-shapes vocabulary (Shape 1=input-side, Shape 2=output-side, Shape 3=structural-rewrite), erfc is not a single-shape instance — it's a composite:
- `erfc = 1 - erf` naïve formula: **Shape 2** (output-side cancellation — `1 - near_1` catastrophic for x near 0)
- fdlibm four-region rational-approximation fix: **Shape 3** (structural rewrite — dedicated polynomials per region; not a transform of the erf computation)
- `exp(-z*z - 0.5625)` precision-splitting trick in erfc2: another **Shape 3** instance (SET_LOW_WORD trick is a functional-identity rewrite of the exponent)

Implication for Sweep 37 recipe design: erfc needs explicit three-shapes decomposition in its recipe doc. The complementary-argument-transform frame is not sufficient to describe it — erfc is the first downstream family that forces the full four-axis recipe metadata schema naturalist's addendum proposed.
