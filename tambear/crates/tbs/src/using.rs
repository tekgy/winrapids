...
/// Precision preference (from using(precision="fp32")).
...
/// NaN handling policy (from using(nan="trim")).
#[derive(Debug, Clone, Copy, PartialEq)]
pub enum NanPolicy {
    /// Skip NaN elements (default for most stats).
    Omit,
    /// Reject: return error/NaN if any NaN present.
    Reject,
...
impl UsingBag {
    ...
    /// NaN policy override.
    pub fn nan_policy(&self) -> Option<NanPolicy> {
        let s = self.get_str("nan")?;
        match s {
            "trim" | "omit" => Some(NanPolicy::Omit),
            "kill" | "reject" | "error" => Some(NanPolicy::Reject),
            s if s.starts_with("fill:") => {
                s[5..].parse::<f64>().ok().map(NanPolicy::Replace)
            }
            _ => None,
        }
    }
...
mod tests {
    ...
    fn set_and_query() {
        ...
        assert_eq!(bag.precision(), Some(Precision::F32));
        assert_eq!(bag.nan_policy(), Some(NanPolicy::Reject));
    }
    ...
    fn later_using_overwrites() {
        ...
        assert_eq!(bag.precision(), Some(Precision::F64));
    }
    ...
    fn nan_fill_parse() {
        ...
        assert_eq!(bag.nan_policy(), Some(NanPolicy::Replace(0.0)));
    }
